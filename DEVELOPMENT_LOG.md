...
状态：已读取用户失败日志、补齐 Relay 容量与 Android 重新配对时序，安装新 Host/Android 并部署公网 Relay；最终 Android 修复连续两轮真实设备回归通过。本轮源码、测试、规范与日志修改未提交、未推送。

根因与完成内容：

- 手机日志保留了 21:37:33、35、38 的三次真实 `Relay authentication_failed: authentication failed`。当时安装的 Host SHA-256 为 `e960615604e9d781c4fd776c7c1d5914c8f3ec20a0dadc03f91669f938a9ef1d`，与已构建的修复版不同，运行状态也没有 ready route 字段。已安装 SHA-256 `b6a9a1dd3697f128e8649ff85dc3a3f25d06f135a39906458220bf4879c83afb` 的 Host 并使用 `codex-rinia` 重启。
- 新 Host 真机配对首先触发 `Relay waiting registration capacity is full`：上轮每设备独立连接未配套调整 Relay 原有的 4 个等待注册限制。Relay 上限改为 17 个等待注册（16 台设备加扫码路由），外部连接上限 68，保持有界且不更改线协议。
- 22:01:59 的重复配对进一步捕获卡片 `RETRYING` / “与 K-NoNa 的连接已断开”，发生在成功回执前。终端批准已轮换凭据，而手机旧连接尚未停止。Android 现在解码二维码后，对同一 Host 先停止并等待旧连接清空，再发送配对请求；成功后使用新凭据连接。配对其他 Host 不在此步骤停止旧连接。
- 增加 opt-in 真机 `PairingConnectionTest`：在设备上用 ML Kit 解码真实终端二维码，调用前台 Activity 的正式 MainViewModel，经过公网 Relay、真实 TLS 与凭据存储，监测卡片全部 ConnectionRuntime 状态。任一错误均使测试失败，连上后继续观察 5 秒。测试支持先建立旧连接再重新配对，不新增生产导入二维码入口，不记录凭据。

验证结果：

- 最终 Android 修复在已有真实连接条件下连续两轮通过。第一轮 22:05:53.698 进入等待批准，22:05:55.483 卡片 Connected（约 1.8 秒）；第二轮 22:06:53.121 至 22:06:55.142（约 2.0 秒）。脚本检测终端 `[y/N]` 后立即输入 y，两轮均无 RETRYING、无卡片错误。
- 较早的未连接条件测试也通过：21:54:43.838 等待批准，21:54:45.776 Connected；该轮在 Android 停止旧连接修复前执行。此前失败的重复配对测试未被计为成功。
- 本轮自动化不覆盖相机光学采集/对焦；二维码图像经测试文件送入设备后由正式 ML Kit 解码。实际配对、认证、凭据轮换与卡片状态来自真机和公网服务。
- Relay 8 项测试、Clippy `-D warnings`、Release 构建通过；Android 单元测试、lintDebug、Debug App 与 instrumentation APK 构建通过，已覆盖安装到设备 `10CD5Q1FAS0007Y`。规范与各仓库 whitespace 检查通过。

部署与 Git 状态：

- 公网 Relay 旧发布目录 `34e0b8707ec8152bbee743ab6e31c9ec9f49eb10`，新目录 `70bd431a096ed5dfd16fc3796db0b2b7a3379380`；新目录名为二进制 SHA-1 产物标识，不是 Git 提交。既有受限部署脚本的配置校验、重启和健康检查通过，保留原发布供回滚。
- 当前源码基线：总控 `1621f2d`、Rust `34e0b87`、Android `15ce870`、协议 `aa98109`。上轮 Rust 修复已在本轮开始前由用户提交；本轮新增修改未提交、未推送，未更新 Submodule 指针。

遗留事项：桌面计划弹窗按用户决定不修改 Codex；首次对话临时标题线程过滤尚待单独真机会话验收。原有 Queue、复杂输入和历史增量验收仍未完成。

下一目标：在 Android 15 公网 Relay 链路验收首轮对话不展示临时标题线程。
